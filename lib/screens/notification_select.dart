...
class _NotificationSelectState extends State<NotificationSelect> {
                    ...
                    child: Column(
                      ...
                      children: [
                        Widgets.getNavBarWithoutProfile(
                            constraints, _back, "Notification"),
                        textWidgetCentered("wähle deine Zeit aus"),
                        textWidgetCentered(
                            "aktuelle Zeit: ${time!.hour.toString()}:${time!.minute.toString()}"),
                        Widgets.getButtonStyleOrange(
                            _setTime, constraints, "Zeit setzen"),
                        SizedBox(),
                      ],
                    ),
                  ),
  ...
  Future<void> setTimeFromStorage() async {
    ...
    await storage.read(key: "notificationtime").then((value) => {
          if (value != null)
            {timeString = value, time = stringToTime(timeString), _renderNew()}
        });
  }
